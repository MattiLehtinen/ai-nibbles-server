var _ = require('lodash'),
    util = require('util'),
    Game = require('./game'),
    GameConnection = require('./socket-connection'),
...
GameCoordinator.prototype._createGame = function(data) {
    ...
    this.emit("game_create", this._game);
...
GameCoordinator.prototype._joinGame = function(player) {
    ...
    this._game.on("apple", function(data) {self._onApple(data);});
...
GameCoordinator.prototype._onApple = function(data) {
    this._gameConnection.sendMessage(ServerMessages.APPLE, data);
    this.emit(ServerMessages.APPLE, data);
};

GameCoordinator.prototype._onGameStart = function(data) {
    this._gameConnection.sendMessage(ServerMessages.START, data);
    this.emit("game_start", {game: this._game});
};
...
GameCoordinator.prototype._onMessage = function(message) {
    ...
    switch (this._gameState()) {
        case Game.States.NONE:
            ...
            break;
        case Game.States.GAME_PLAY:
            switch (messageType) {
                case ClientMessages.CONTROL:
                    this._game.control(this._player, data);
            }
            break;
    }
